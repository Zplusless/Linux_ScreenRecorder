...
# 2. 采用多进程结构，将图像采集和视频写入分开，保障图像采集的频率（IO不受GIL的影响，多线程也可以）
# 3. 主进程和子进程的数据传递，使用multiprocessing.Manager().dict(),构建共享内存的字典
#     - https://docs.python.org/3.6/library/multiprocessing.html#sharing-state-between-processes
# 4. 除了截图信息需要走共享内存，子进程的while循环判断条件也要走共享内存。
# 5. VideoGet中录屏需要warm up，在init阶段拿到的帧率普遍偏小（几十fps），因而限制了整体的fps，warm up之后能到300fps
# 6. 截图进程和主进程之间信息传递不应该使用队列，因为如果子进程截图fps大于主进程写入视频的fps，会导致写入的帧不是当前截图的帧，容易产生漂移。
